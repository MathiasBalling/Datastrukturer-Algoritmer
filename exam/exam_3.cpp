...
int main() {
  ...
  string maxTask = "";
  string minTask = "";
  string finalMaxSlack = "";
  int minVarighedAktuelEvent = numeric_limits<int>::max();
  ...
  while (true) {
    while (indeks < tabel.size() && tabel[indeks].getEvent() == aktuelEvent) {
      ...
      if (minVarighedAktuelEvent > tabel[indeks].getDuration()) {
        minVarighedAktuelEvent = tabel[indeks].getDuration();
        minTask = tabel[indeks].getTask();
      }

      indeks++;
    }
    int slackAktuelEvent = maxVarighedAktuelEvent - minVarighedAktuelEvent;
    if (slackAktuelEvent > maxSlack) {
      maxSlack = slackAktuelEvent;
      finalMaxSlack = minTask;
    }

    laengdeKritiskVej += maxVarighedAktuelEvent;
    kritiskVej += maxTask;
    maxVarighedAktuelEvent = 0;
    minVarighedAktuelEvent = numeric_limits<int>::max();
    maxTask = "";
    minTask = "";
